...
npm run provider -- facebook-pages            # regenerate this repo
npm run provider -- facebook-pages --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/facebook-pages-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/facebook-pages-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/facebook-pages-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-facebook-pages` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/facebook-pages/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/facebook-pages` |
| npm | `@particle-academy/facebook-pages-ui`, `@particle-academy/facebook-pages-js` |
| Packagist | `particle-academy/facebook-pages-php` |
| PyPI | `fancy-facebook-pages` |
...
**Default to plain HTTP.** A Facebook Pages SDK is third-party code and is subject to
