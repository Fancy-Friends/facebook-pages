/*
 * Facebook Pages — the published npm packages.
 *
...
import { facebookPagesFaker } from "@particle-academy/facebook-pages-js";
import { FACEBOOK_PAGES_KINDS } from "@particle-academy/facebook-pages-ui";
 ...
 */
const resolved = import.meta.resolve("@particle-academy/facebook-pages-js");
...
for (const { operation, config, expected } of GOLDENS) {
  const faked = facebookPagesFaker(operation, fakeRequest("facebook_pages", operation, config));
...
assert.equal(FACEBOOK_PAGES_KINDS.length, 1);
for (const kind of FACEBOOK_PAGES_KINDS) {
...
console.log(`  ok   ui kinds resolve from ${"@particle-academy/facebook-pages-ui"}`);
