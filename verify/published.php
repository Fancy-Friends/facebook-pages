...
/*
 * Facebook Pages — the published Composer package.
 *
...
if (! is_file($autoload)) {
    ...
    fwrite(STDERR, '    composer require particle-academy/facebook-pages-php'.PHP_EOL);
    exit(2);
}
...
foreach ($goldens as $golden) {
    ...
    $fake = new FakeValues(FakeValues::seedForCall('facebook_pages', $operation, $config));
    $faked = FacebookPagesFaker::respond($operation, ['config' => $config, 'fake' => $fake]);
